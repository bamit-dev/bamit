...
unsigned int nStakeMinAge = 8 * 60 * 60; // 12 hours
...
const string strMessageMagic = "BamitCoin Signed Message:\n";
...
// Bamit Miner's Rewards
int64_t GetProofOfWorkReward(int64_t nFees)

    {
    int64_t nSubsidy = 0 * COIN;

    if (pindexBest->nHeight < 1) //Premine 1% of total coins
        nSubsidy = 9473 * COIN; 
    
    else if(pindexBest->nHeight < 10) {
        nSubsidy = 0 * COIN; // fair start
    }   
          
    else if(pindexBest->nHeight < 2880) { //Rewards blocks start here at block 11
        nSubsidy = 101 * COIN;
    }
...
static const int POW_BLOCK_COUNT = 8640;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BamitCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "BamitCoin New Hash 6/17/15 Hillary Lies (Again)";
        CTransaction txNew;
        txNew.nTime = 1434732105;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xf73f50a5adc0d97c52a1adbd620b1041a56df8d725e182f851693912bd0da010"));
        block.print();
